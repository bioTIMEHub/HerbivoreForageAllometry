...
area.ci <- list(rep('', 5))
univars <- c('Sizem', 'Speciesm', 'SSm', 'Dietm', 'Funcm')
...
sp_names <- c("A. nigricauda", "C. striatus", "N. unicornis", "Z. scopas","Ch. spilurus", "Sc. frenatus", "Sc. rivulatus", "S. doliatus", "S. vulpinis")
diet_names <- c('EAM', 'macroalgae', 'detritus', 'corticated algae', 'cyanobacteria')
func_names <- c('excavator', 'scraper', 'grazer/detritivore', 'browser')
ss_names <- c('single', 'pair', 'single sp school', 'mixed-sp school')

base <- ggplot() +
  theme_classic(base_size = 13, base_family = 'Helvetica') + ylab(element_blank())
# Body size
...
A.body <- base +
  ...
  geom_point(data=forage.data, aes(y=Area, x = Size), size = 2, alpha = 0.8, shape =21) +
...
A.sp <- base +
  ...
  geom_violin(data = forage.data, aes(x=Species, y=Area), fill = 'grey90', color = 'grey80') +
...
A.ss <- base +
  geom_hline(aes(yintercept = mean(forage.data$Area)), linetype='dashed', alpha = 0.6, size=0.6) +
  geom_violin(data = forage.data, aes(x=SS, y=Area), fill = 'grey90', color = 'grey80') +
  ...
  xlab('Social structure') + scale_x_discrete(labels=ss_names)

A.diet <- base +
  geom_hline(aes(yintercept = mean(forage.data$Area)), linetype='dashed', alpha = 0.6, size=0.6) +
  geom_violin(data = forage.data, aes(x=Diet, y=Area), fill = 'grey90', color = 'grey80') +
  ...
  xlab('Diet') + scale_x_discrete(labels=diet_names)

A.func <- base +
  geom_hline(aes(yintercept = mean(forage.data$Area)), linetype='dashed', alpha = 0.6, size=0.6) +
  geom_violin(data = forage.data, aes(x=Func, y=Area), fill = 'grey90', color = 'grey80') +
  ...
  xlab('Functional group') + scale_x_discrete(labels=func_names)

(A.body + A.sp) / (A.ss + A.diet + A.func) & scale_y_continuous(trans='log', limits = c(1,1000), breaks = c(1,5,10,50,100,500,1000))
...
T.body <- base +
  geom_ribbon(data = tort.ci[[1]] %>% bind_cols(., Size = forage.data$Size), aes(ymax=upper, ymin=lower, x=Size),
              fill = 'transparent', color = 'black', linetype='dashed') +
  geom_point(data=forage.data, aes(y=Tort, x = Size), size = 2, alpha = 0.8, shape =21) +
...
T.sp <- base +
  ...
  geom_violin(data = forage.data, aes(x=Species, y=Tort), fill = 'grey90', color = 'grey80') +
...
T.ss <- base +
  geom_hline(aes(yintercept = mean(forage.data$Tort)), linetype='dashed', alpha = 0.6, size=0.6) +
  geom_violin(data = forage.data, aes(x=SS, y=Tort), fill = 'grey90', color = 'grey80') +
...
T.diet <- base +
  geom_hline(aes(yintercept = mean(forage.data$Tort)), linetype='dashed', alpha = 0.6, size=0.6) +
  geom_violin(data = forage.data, aes(x=Diet, y=Tort), fill = 'grey90', color = 'grey80') +
...
T.func <- base +
  geom_hline(aes(yintercept = mean(forage.data$Tort)), linetype='dashed', alpha = 0.6, size=0.6) +
  geom_violin(data = forage.data, aes(x=Func, y=Tort), fill = 'grey90', color = 'grey80') +
...
(T.body + T.sp) / (T.ss + T.diet + T.func) & scale_y_continuous(trans='log', limits = c(1,100), breaks = c(1,5,10,20,50,100))

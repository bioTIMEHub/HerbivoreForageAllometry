...
#Finding CIs
dol<-predict.glm(VarIntmodel,newdata=doliatus1, type=c("response"),se.fit=TRUE)
dolupr <- dol$fit + (critval * dol$se.fit)
dollwr <- dol$fit - (critval * dol$se.fit)

fren<-predict.glm(VarIntmodel,newdata=frenatus1, type=c("response"),se.fit=TRUE)
frenupr <- fren$fit + (critval * fren$se.fit)
frenlwr <- fren$fit - (critval * fren$se.fit)

nig<-predict.glm(VarIntmodel,newdata=nigricauda1, type=c("response"),se.fit=TRUE)
nigupr <- nig$fit + (critval * nig$se.fit)
niglwr <- nig$fit - (critval * nig$se.fit)

riv<-predict.glm(VarIntmodel,newdata=rivulatus1, type=c("response"),se.fit=TRUE)
rivupr <- riv$fit + (critval * riv$se.fit)
rivlwr <- riv$fit - (critval * riv$se.fit)

sco<-predict.glm(VarIntmodel,newdata=scopas1, type=c("response"),se.fit=TRUE)
scoupr <- sco$fit + (critval * sco$se.fit)
scolwr <- sco$fit - (critval * sco$se.fit)

sor<-predict.glm(VarIntmodel,newdata=sordidus1, type=c("response"),se.fit=TRUE)
sorupr <- sor$fit + (critval * sor$se.fit)
sorlwr <- sor$fit - (critval * sor$se.fit)

stri<-predict.glm(VarIntmodel,newdata=striatus1, type=c("response"),se.fit=TRUE)
striupr <- stri$fit + (critval * stri$se.fit)
strilwr <- stri$fit - (critval * stri$se.fit)

uni<-predict.glm(VarIntmodel,newdata=unicornis1, type=c("response"),se.fit=TRUE)
uniupr <- uni$fit + (critval * uni$se.fit)
unilwr <- uni$fit - (critval * uni$se.fit)

vul<-predict.glm(VarIntmodel,newdata=vulpinus1, type=c("response"),se.fit=TRUE)
...
#No1 frenatus
plot(VarInt~Size, data=frenatus,las=1, type="p",bty="l",pch=16, cex =0.9,
     xlab="",ylab="",tck=0.03, xlim=c(5,35), ylim=c(0.1,250),log="y")
...
#No2 nigricauda
plot(VarInt~Size, data=nigricauda,las=1, type="p",bty="l",pch=16, cex =0.9,
     xlab="",ylab="",tck=0.03, xlim=c(5,35), ylim=c(0.1,250),log="y")
...
#No3 doliatus
plot(VarInt~Size, data=doliatus,las=1, type="p",bty="l",pch=16, cex =0.9,
     xlab="",ylab="",tck=0.03, xlim=c(5,35), ylim=c(0.1,250),log="y")
...
#No4 rivulatus
plot(VarInt~Size, data=rivulatus,las=1, type="p",bty="l",pch=16, cex =0.9,
     xlab="",ylab="",tck=0.03, xlim=c(5,35), ylim=c(0.1,250),log="y")
...
#No5 scopas
plot(VarInt~Size, data=scopas,las=1, type="p",bty="l",pch=16, cex =0.9,
     xlab="",ylab="",tck=0.03, xlim=c(5,35), ylim=c(0.1,250),log="y")
...
#No6 vulpinus
plot(VarInt~Size, data=vulpinis,las=1, type="p",bty="l",pch=16, cex =0.9,
     xlab="",ylab="",tck=0.03, xlim=c(5,35), ylim=c(0.1,250),log="y")
...
#No7 sordidus
plot(VarInt~Size, data=sordidus,las=1, type="p",bty="l",pch=16, cex =0.9,
     xlab="",ylab="",tck=0.03, xlim=c(5,35), ylim=c(0.1,250),log="y")
...
#No9 unicornis
plot(VarInt~Size, data=unicornis,las=1, type="p",bty="l",pch=16, cex =0.9,
     xlab="",ylab="",tck=0.03, xlim=c(5,60), ylim=c(0.1,250),log="y")
